...
#define OTA_UPGRADE_DOWNLOADED_FILE_VERSION 0x01010108                              /* The attribute indicates the file version of the downloaded firmware image on the device */
...
typedef struct {
    uint8_t text[32];   // CHAR_STRING: max 31 bytes + 1 length
    uint16_t text_id;
    uint16_t count;       // U16
    uint16_t count_id;
} custom_attr_data_t;
